...
For a quick linux command line refresher you can visit the following site [Ryan's Linux & Bash Tutorial ](https://ryanstutorials.net/linuxtutorial/). [Section 1 covers the command line](https://ryanstutorials.net/linuxtutorial/commandline.php), and [section 2 covers navigation](https://ryanstutorials.net/linuxtutorial/navigation.php).
...
* A `data/` subdirectory: This directory contains data sets of the project. Since Git is not as efficient with non-text files, and GitLab has storage limits, you should only put small data sets in that directory (<2-3 MB). These can be geopackages, small rasters, csv files.
...
* Finally, a `README.md` file should be included, this file should contain a description of your project, its name and the name of the authors, along with a description of what other packages your package needs to function correctly. 
...
# Load datasets 
wageningen_ndvi <- rast('data/wag_NDVI.tif')
...
# Load datasets 
postboxes = gpd.read_file('data/postbox_locations.shp')
# Then the actual commands
```

## Working directory, relative and absolute file paths
*At the end of the following section you should be able to explain the difference between __root directory, working directory, absolute path, relative path__* 

In the R and Python examples above we load the datasets by indicating the file location from the data directory `"data/postbox_locations.shp"`. This is an example of using **relative** file paths. We don't include the location of the project directory itself (`"C:/Geoscripting/myproject/data/postbox_locations.sph"`). The reason why they are called *relative* is because they are *relative* to the **working directory**, which is the directory *from which the script is called* (`"C:/Geoscripting/myproject"`). By convention, the working directory is the same as the location of the script itself. This means that you can simply assume that whoever runs your script, will run it from the directory that your script is located.
