...
```{r, echo=TRUE, fig=TRUE, include=TRUE}
...
<!--
TODO: Make the project section more concrete by creating a projects that contains the function we have written previously
-->
...
Have you ever worked on a project and ended up having so many verions of your work that you didn't know which one was the lattest, and what were the differences between the versions? Probably yes, we all experienced that. Then you need to use version control (also called revision control). You will quickly understand that although it is designed primarily for big software development projects, being able to work with version control can be very helpful for scientists as well.

The video below explains some basic concepts of verion control and why the benefits of using it are.
...
* `Commit`: Consists of adding the latest changes to the repository. When you **commit** your changes, it is like putting a milestone in your progress. That milestone describes what have been changed, why and when. In the future you can always go back to that milestone.
* `Push` : Add previous changes committed to the local repository, to the remote repository.
* `Clone` : Copy the content of a remote repository locally.
...
<center><img src="figs/github_screenCapture.png" alt="Drawing" style="width: 700px"/></center>
