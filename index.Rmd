...
pagetitle: "Week 1, Lesson 1: Version control"
output:
...
If you are a student following the course at Wageningen University (WUR), **please read** the information on [Brightspace](https://brightspace.wur.nl). All course-specific information and exercises can be found there. Information on Brightspace overrule any information written in these pages, so **please read it carefully** and **check it often**. You will also find all the information on deliverables and exercises there.
```
...
```{block, type="alert alert-info"}
**In case you can't get the VDI running successfully** (and **only** in that case, so hopefully you don't need to do this!), there is one alternative: if you know what you are doing, you can install Linux on your own laptops or external hard disks. See a [theoretical overview of running Linux on your own hardware](../Intro2Linux/installation.html) and [Linux system setup](../system_setup/index.html).
...
Throughout the Geoscripting course, for hosting our code, we will be using [GitHub](https://github.com), the most popular Git hosting platform.
