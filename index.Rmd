...
output:
  knitrBootstrap::bootstrap_document:
    title: "Tutorial 1: Linux & version control"
    theme: "simplex"
...
```{block, type="alert alert-info"}
**In case you can't get the VDI running successfully** (and **only** in that case, so hopefully you don't need to do this!), there is an alternative: we have the possibility of providing access to a SURFsara cloud VM setup. [See this page for instructions on gaining access to the SURFsara cloud VM](../Intro2Linux/surfsara_tutorial.html).

Another alternative (also if you are not following the course at WUR), is to install Linux on your own laptop or external hard disk. See a [theoretical overview of running Linux on your own hardware](../Intro2Linux/installation.html) and [Linux system setup](../system_setup/index.html).

**The VDIs are strongly recommended**. If you go for installing Linux yourself, the systems need to be set up manually and we do not have the time and manpower to support every student with this.
...
Once you have everything ready, login into your Linux VM, try out RStudio, and also open QGIS. Explore the environment a little to get used to it.
