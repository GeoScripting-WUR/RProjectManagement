...
subtitle: "Week 1, Lesson 5: Carrying out your R project"
author: "Loïc Dutrieux, Jan Verbesselt, Johannes Eberenz"
date: "7/01/2015, Last updated: `r format(Sys.time(), '%d %B, %Y')`"
output:
  html_document:
    number_sections: yes
    css: custom.css
    ...
    toc_depth: 2
---
# Carrying out your R project 
## Introduction

Good morning! During the previous lecture, you saw some general aspects of the R language, such as an introduction to the syntax, object classes, reading of external data and function writing. 
...
- Now try the same with the RStudio tools: 
```{r, eval=FALSE}
fu <- function(y) { 
    foo(y)
}
...
#fu(2) 
```
- The error is now nested one level deeper. RStudio will offer to show the traceback and rerunning the the problematic code with buttons in the console. Chose "Rerun with Debug" and use the little green "Next" button to go line by line through the code. The traceback appears in a separate pane on the right.  
...
Further, R has a more elaborated debugging mode that can be activated by calling `debug()`. Although the use of `debug()` is not covered in this lesson, fell free to explore by yourself these debugging functionality. Refer to the reference section of this document for further information on function debugging.
