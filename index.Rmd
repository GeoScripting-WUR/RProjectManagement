...
In the VirtualBox, click *Show Apps* at the bottom left and either click on *Utilities* → *Passwords and Keys*, or just start typing *Passwords* in the *type to search* searching bar and hit Enter.
...
You also need to set up two-factor authentication by going to [your profile page](https://git.wur.nl/-/profile/account). 
...
Open it from *Show Apps* at the bottom left of VirtualBox.
...
To see a list of files with pending changes from the command line, use `git status` while in a git repository folder. To see what exactly changed in each of these files, use `git diff`. For example:
