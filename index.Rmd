...
If you are a student following the course at Wageningen University (WUR), **please read** the information on [Brightspace](https://brightspace.wur.nl). All course-specific information can be found there. Information on Brightspace overrule any information written in these pages, so **please read it carefully** and **check it often**. You will also find all the information on deliverables and exercises there.

Throughout the whole course, we will be working in a Linux environment, and all of **the material has only been tested on (and assumes) a Linux environment**. While we will get into the details of what Linux is and why we use it in the next lesson, it is easiest if you **start using it also for this lesson**, as it will save you the trouble of setting up your version control twice. Every WUR student gets access to one Linux virtual machine. To find information on how to access the virtual machine, see the course information on Brightspace.
```
