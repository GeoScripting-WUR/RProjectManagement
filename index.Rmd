...
```{block, type="alert alert-info"}
**Note**: If you are taking the course externally and do not have access to the WUR GitLab instance, you can use the [public GitLab instance](https://gitlab.com) or [GitHub](https://github.com) instead.
```
...
Now that you have a remote repository, it's time to create a local repository that links to it! Open the main page of your new project and copy the SSH address of your new repository by copying the URL in the *SSH* box located below the description of your project (if it's showing *HTTPS*, click *HTTPS* and switch to *SSH*).
...
Before you start using Git, you should tell it what your name and email address it. You need to do that only once per Git installation. You should go to *Edit* → *Options...* and fill out the *Global (All Repositories)* options *User Name* and *Email Address*. These will be displayed on GitLab.
...
1. Whether the function works as intended and handles a variety of input
2. Whether the function is readable, complete, type consistent, throws meaningful errors and warnings when appropriate.
3. The structure of your project and the appropriate use of git

For bonus points, try to make the function as short as possible: can you come up with a solution without using any `if` statements?
