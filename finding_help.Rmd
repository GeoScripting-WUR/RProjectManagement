...
Generative AI, such as ChatGPT, has recently been gaining popularity. Generative AI models can be interacted with by asking it questions, including questions about programming. The AI responds by providing examples of code, explanations about what the code does, and how to run it. Of course, most AI solutions are not limited to code and will also answer questions on history, biology, quantum mechanics, and will even play Dungeons and Dragons with you, including throwing dice.
...
Most of the AI models are also paid per query, and therefore you have only a limited amount of questions you can ask to it.
...
The first question is critical to help you learn.
...
You also don't know if the code is efficient or not.
...
Note that when you use generative AI autocompletion, you need to be extra careful, as it will always provide you code. It is useful to turn off AI autocompletion during the course, so you can think about the code yourself before the autocompletion provides the code for you.
The same applies as with code from chatbots: never accept the code as-is, always check what the functions are and what they do, and try to rewrite it to make sense for you.
...
* [Amazon Q Developer](https://aws.amazon.com/q/developer/) - Amazon code suggestion AI, free to use, but works only with some code editors.
...
A server and open-weight model list can be find on the [Ollama](https://ollama.com/) website.
A web interface that links to it can be found on [Open WebUI](https://docs.openwebui.com/).
...
To enable Copilot in RStudio, again go to *Tools* → *Global Options...* in the top menu bar. A menu will appear where you should change the setting according to the settings shown in the image below. Make sure to set the Copilot Completions to **manually**. Now you can enable autocompletion when you need it with **Ctrl + Backslash**.

![Global Options](../Scripting4GeoIntro/figs/copilot_Rstudio.png)
